...
#include "AGuiOpenCV/AGuiOpenCVQuote.h"

#include "MfcLib/AGuiFun/AGuiPath.h"
#include "AGuiOpenCV/Base/CvvImage.h"
...
BEGIN_MESSAGE_MAP(CTestOpevCvDlg, CDialogEx)
	...
	ON_COMMAND(ID_FLASH_FADEIN,OnImgFlashFadein)
	ON_COMMAND(ID_FLASH_LTL,OnImgFlashLeftTopLine)
	ON_COMMAND(ID_FLASH_LR,OnImgFlashLeftRight)
	ON_COMMAND(ID_FLASH_MC,OnImgFlashMidCircle)
	ON_COMMAND(ID_FLASH_MR,OnImgFlashMidRect)
	ON_COMMAND(ID_FLASH_UD,OnImgFlashUpDown)
	ON_COMMAND(ID_FLASH_UDM,OnImgFlashUpDownMove)
	ON_COMMAND(ID_FLASH_UL,OnImgFlashUpLeft)
...
void CTestOpevCvDlg::OnImgReverse()
{
	if(m_ImgSrc)
	{
		...
		m_ImgRes = ImgTransReverse(m_ImgSrc);
		Invalidate();
	}
}
void CTestOpevCvDlg::OnImgLine()
{
	if(m_ImgSrc)
	{
		...
		m_ImgRes = ImgTransLinear(m_ImgSrc,2,-90);
		Invalidate();
	}
}
void CTestOpevCvDlg::OnImgLineSub()
{
	if(m_ImgSrc)
	{
		...
		m_ImgRes = ImgTransSubLinear(m_ImgSrc,50,50,150,255);
		Invalidate();
	}
}
...
void CTestOpevCvDlg::OnImgThres()
{
	if(m_ImgSrc)
	{
		...
		m_ImgRes = ImgTransThreshold(m_ImgSrc,100,200);
		Invalidate();
	}
}
...
void CTestOpevCvDlg::OnImgFlashFadein()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowFadein(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowFadein(m_ImgSrc);
	}
}

void CTestOpevCvDlg::OnImgFlashLeftTopLine()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowLeftTopLine(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowLeftTopLine(m_ImgSrc);
	}
}
void CTestOpevCvDlg::OnImgFlashLeftRight()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowLeftRight(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowLeftRight(m_ImgSrc);
	}
}
void CTestOpevCvDlg::OnImgFlashMidCircle()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowMidCircle(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowMidCircle(m_ImgSrc);
	}
}
void CTestOpevCvDlg::OnImgFlashMidRect()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowMidRect(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowMidRect(m_ImgSrc);
	}
}
void CTestOpevCvDlg::OnImgFlashUpDown()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowUpDown(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowUpDown(m_ImgSrc);
	}
}
void CTestOpevCvDlg::OnImgFlashUpDownMove()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowUpDownMove(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowUpDownMove(m_ImgSrc);
	}
}
void CTestOpevCvDlg::OnImgFlashUpLeft()
{
	if(m_ImgSrc)
	{
		if(m_ImgRes)
		...
		m_ImgRes=cvCloneImage(m_ImgSrc);
		ShowUpLeft(m_ImgSrc,this,IDC_STATIC_IMG_RES);
		//ShowUpLeft(m_ImgSrc);
	}
}